...
namespace GraniteHouse.Areas.Admin.Controllers
{
    ...
    public class AdminUsersController : Controller
    {
        ...
        private async void GetRolesListBySelectedUser(string id)
        {
            ...
            var selectedRolesIds = _db.UserRoles.Where(p => p.UserId == id).Select(p => p.RoleId).ToList();
            
            // Get Roles list from Database
            var rolesFromDB = await _db.Roles.ToListAsync();
            ...
            foreach (var itemRole in rolesFromDB)
            {
                ...
                if (selectedRolesIds.Contains(itemRole.Id))
                {
